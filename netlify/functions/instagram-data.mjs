...
async function refreshTokenIfDue(connection, supabase) {
  ...
  if (expiresAt - Date.now() > sevenDays || Date.now() - connectedAt < oneDay) return connection.access_token;
...
export default async function handler(request) {
  ...
  try {
    ...
    const { data: connection, error: connectionError } = await supabase.from("instagram_connections")
    ...
    if (action === "profile") return json({ profile: await instagramGet("/me?fields=user_id,username", accessToken) });
    return json({ media: await instagramGet(`/${encodeURIComponent(connection.instagram_user_id)}/media?fields=id,caption,media_type,media_product_type,media_url,permalink,timestamp&limit=25`, accessToken) });
  } catch (error) {
    console.error("instagram-data", error?.status || "", error?.payload || error);
    if (error?.status === 401 || error?.status === 190) return json({ error: "Instagram authorization is no longer valid. Please reconnect Instagram." }, 401);
