...
import { scanSource } from "./tournament-scan.mjs";
...
export default async function scheduledHandler() {
  let supabase;
  try { supabase = createAdminSupabaseClient(); } catch { return new Response("Tournament scanner is not configured.", { status: 503 }); }
  ...
  for (const scan of scans ?? []) {
    ...
    try {
      const checked = await scanSource(scan.source_url);
      await supabase.from("tournament_scans").update({
        ...checked.extracted,
        source_hash: checked.source_hash,
        status: "checked",
        detected_changes: scan.source_hash && scan.source_hash !== checked.source_hash
