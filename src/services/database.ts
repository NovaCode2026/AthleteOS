...
function extractDatabaseMessage(error: unknown) { if (error instanceof Error) return error.message; if (error && typeof error === "object" && "message" in error) { const message = (error as { message?: unknown }).message; if (typeof message === "string" && message.trim()) return message; } return "The database request failed."; }
function toDatabaseError(resource: Resource, action: string, error: unknown) { const detail = extractDatabaseMessage(error); const safeDetail = import.meta.env.DEV ? ` ${detail}` : action === "save" ? " Please check the fields and try again." : " Please try again later."; return new Error(`Unable to ${action} ${resource}.${safeDetail}`); }
...
export async function upsertRow<T extends Record<string, unknown>>(resource: Resource, values: T, options: { onConflict?: string } = {}) { const table = TABLES[resource]; const query = options.onConflict ? requireSupabase().from(table).upsert(values, { onConflict: options.onConflict }) : requireSupabase().from(table).upsert(values); const { data, error } = await query.select().single(); if (error) throw toDatabaseError(resource, "save", error); return data; }
export async function insertRow<T extends Record<string, unknown>>(resource: Resource, values: T) { const table = TABLES[resource]; const { data, error } = await requireSupabase().from(table).insert(values).select().single(); if (error) throw toDatabaseError(resource, "save", error); return data; }
